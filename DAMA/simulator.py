...
from DAMA.constants import SimulationConfig
...
def get_simulation_config(target_moe: float):
    ...
    """
    Z_SCORE = SimulationConfig.Z_SCORE_95
    P_VAL = SimulationConfig.P_VAL_WORST_CASE
    ...
    # Tiers = (Name, Iterations, Multiplier)
    # Using config constants for consistency
    return [
        ("Casual", SimulationConfig.TIER_CASUAL[0], clean_n(base_n * SimulationConfig.TIER_CASUAL[1])),
        ("Advanced", SimulationConfig.TIER_ADVANCED[0], clean_n(base_n * SimulationConfig.TIER_ADVANCED[1])),
        ("Competitive", SimulationConfig.TIER_COMPETITIVE[0], clean_n(base_n * SimulationConfig.TIER_COMPETITIVE[1]))
    ]
...
def run_single_match(p1_deck_id: int, p2_deck_id: int, specs1: dict, specs2: dict, turn_iterations: int = 100,
                     verbose: bool = False, registry: MetaRegistry | None = None) -> tuple[int | None, MetaRegistry]:
    if not specs1 or not specs2: return None, registry if registry else MetaRegistry()
    registry = registry if registry else MetaRegistry()
    ...
    MAX_TURNS = SimulationConfig.MAX_MATCH_TURNS
    stuck_counter = 0
    last_turn_hash = ""
    ...
    while game.winner == Winner.NONE and game.turn < MAX_TURNS:
        ...
        # Anti-Stuck Logic
        if stuck_counter > SimulationConfig.STUCK_THRESHOLD_SOFT:
        ...
        tree = MCTSTree(game, registry=registry)
        ...
        if not best_node:
            ...
            if stuck_counter > SimulationConfig.STUCK_THRESHOLD_HARD: break
            continue
        ...
        if game.players[0].currentHP / max(1, game.players[0].currentDurability) < SimulationConfig.COMEBACK_HP_RATIO: p1_low_hp = True
        if game.players[1].currentHP / max(1, game.players[1].currentDurability) < SimulationConfig.COMEBACK_HP_RATIO: p2_low_hp = True
...
def run_batch(batch_args):
    ...
    for i in range(n):
        try:
            win, _ = run_single_match(p1, p2, specs1, specs2, turn_iterations=iterations, verbose=verbose, registry=registry)
...
def main():
    ...
    for tier_name, iterations, n_games in tiers:
        ...
        # 1. Start Registry for this Tier
        tier_registry = MetaRegistry()

        tasks = []
        BATCH_SIZE = SimulationConfig.BATCH_SIZE  # Keep 1 for smooth progress bar updates
